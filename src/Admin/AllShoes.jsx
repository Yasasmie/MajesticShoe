  ...
  const saveEdit = async (id) => {
    try {
      const numericPrice = Number(
        editData.price.toString().replace(/[^0-9.]/g, "")
      );
      const numericStock = Number(editData.stock);
      ...
      const updatedObj = {
        ...
        images: editData.imagesText
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
        sizes: editData.sizesText
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
        colors: editData.colorsText
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
      };
        ...
        <div className="max-w-7xl mx-auto">
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-serif italic text-white/90">
                Catalog Archive
              </h1>
            </div>
          ...
          {!isAdmin ? (
            ...
            <div className="space-y-8">
              {error && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-[10px] uppercase tracking-widest flex items-center gap-2">
                  <X size={12} /> {error}
                </div>
              )}
...
function ShoeCard({
  ...
  if (isEditing) {
    return (
      <div className="col-span-2 lg:col-span-2 bg-[#0F0F0F] border border-red-500/30 rounded-[32px] p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <EditInput
            ...
            onChange={(v) => setEditData({ ...editData, stock: v })}
          />
          <EditInput
            label="Category"
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <EditInput
                ...
                onChange={(v) => setEditData({ ...editData, sizesText: v })}
              />
              <EditInput
                label="Colors"
                value={editData.colorsText}
                onChange={(v) => setEditData({ ...editData, colorsText: v })}
              />
            </div>
            <EditInput
            ...
            <EditInput
              label="Tag"
              value={editData.tag}
              onChange={(v) => setEditData({ ...editData, tag: v })}
            />
          </div>
        </div>
        <div className="flex gap-3 mt-8">
          <button
            onClick={onSave}
            className="flex-1 bg-red-600 py-3 rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-red-500 transition-all shadow-lg shadow-red-900/20"
          >
            Save Changes
          </button>
          <button
            onClick={onDelete}
            className="px-5 bg-white/5 border border-white/10 rounded-xl text-red-500 hover:bg-red-500/10"
          >
            <Trash2 size={16} />
          </button>
        </div>
    ...
    <div className="group relative bg-white/[0.02] border border-white/5 rounded-[24px] md:rounded-[32px] overflow-hidden transition-all duration-500 hover:border-white/20 hover:-translate-y-1 active:scale-95">
      ...
      <div className="aspect-[4/5] overflow-hidden bg-neutral-900 relative">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          <span
            className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter border ${
              categoryColors[shoe.category?.toLowerCase()] ||
        ...
        {outOfStock && (
          <div className="absolute top-3 right-3">
            <span className="px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter bg-neutral-900/90 text-yellow-300 border border-yellow-400/70">
              Out of Stock
            </span>
          </div>
        )}
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3">
          <button
            onClick={onStartEdit}
            className="p-3 bg-white rounded-full text-black hover:bg-red-600 hover:text-white transition-all transform translate-y-4 group-hover:translate-y-0"
          >
            <Edit3 size={16} />
          </button>
          <button
            onClick={onDelete}
            className="p-3 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-red-600 transition-all transform translate-y-4 group-hover:translate-y-0 delay-75"
          >
            <Trash2 size={16} />
          </button>
        </div>
      ...
      <div className="p-3 md:p-5">
        <div className="flex justify-between items-start mb-1">
          <h3 className="text-[11px] md:text-sm font-medium text-white/90 line-clamp-1 truncate">
            {shoe.name}
          </h3>
        ...
        <p className="text-red-500 font-mono text-[10px] md:text-xs mb-2">
          {shoe.price}
        </p>
